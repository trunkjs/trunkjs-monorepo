...
## Öffentliche Paket-Einstiegspunkte

Für alle Code-Pakete dieses Projekts liegt der öffentliche TypeScript-Einstiegspunkt `index.ts` direkt im jeweiligen Paket-Hauptverzeichnis neben `package.json`, niemals unter `src/`; öffentliche Style-Einstiegspunkte wie `index.scss` oder `index.css` liegen ebenfalls dort, während Implementierung und komponenteninterne Styles unter `src/` bleiben. Workspace-Aliase, Build-Einstiegspunkte, TypeScript-Includes, Deklarationsausgabe und Paket-Assets müssen diese Struktur verwenden, damit Pakete und ihre öffentlichen Styles direkt aus einem Quell-Workspace eingebunden werden können; ein zusätzlicher Root-Reexport eines `src/index.ts` erfüllt diese Regel nicht, und Pakete ohne eigene öffentliche Styles benötigen keine leere Style-Datei.
