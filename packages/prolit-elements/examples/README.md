...
The Prolit `@click` expression is best for actions on nodes generated by the scope template. `on()` and `@Listen` are for DOM and application events outside that expression. `ProlitElement` already includes `EventBindingsMixin`; you do not wrap it again. Event callbacks registered in the constructor attach on connection, pause on disconnect, and reattach once on reconnect. Late calls attach immediately. `on()` returns `off()` for permanent removal, including future reconnects.
