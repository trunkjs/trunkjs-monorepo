...
| **Prolit Zielentwurf** | Scope und Template zusammen; expliziter Einfügepunkt mit `prolit(scope, fallback)` | unabhängige Scopes, Directive-Lifecycle, `scopeResource` und `scopeAction` | auch in vorhandenen Komponenten verwendbar; `ProlitElement` ist optionaler Komfort, die API noch zu bauen |
...
Die [nummerierten SPA-Beispiele](examples/README.md) prüfen die Empfehlung zuerst am vollständigen Lade-/Auswahlablauf und seiner DOM-Verbindung, dann an Dialog, austauschbarem Inhalt, Suche, Details, Live-Daten und Fallback-Grenzen. Der Gewinn liegt in wiederkehrenden Zustandsnamen und sichtbaren Auslösern. Die verbleibenden Kosten sind bewusst benannt: callable Action-Generics und Lifecycle-Bindung für den Bibliotheksentwickler; explizite Parameter, ein kleiner Nextrap-Dismiss-Adapter und fehlender Template-Typcheck für den Anwendungsentwickler. Eine weitere Abstraktion ist erst gerechtfertigt, wenn sie diese konkreten Stellen verbessert.

Der universelle Anschluss verringert die Zahl notwendiger Adapterklassen. Seine Kosten liegen im Kern: Scope-Wechsel, doppelte Einbindung, Reconnect und unabhängige Part-Updates müssen eindeutig funktionieren. Ein normales Lit-Template als Fallback verbessert die schrittweise Einführung; es darf aber einen Programmierfehler im vorhandenen Scope nicht verdecken. Ein Template-Typcheck und eine Alternative zur dynamischen Codeauswertung bleiben davon unabhängige Verbesserungen.
