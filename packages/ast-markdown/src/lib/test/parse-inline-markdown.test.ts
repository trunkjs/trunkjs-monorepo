...
describe('parse_inline_markdown – edge-cases', () => {
  it('formats emphasis / strong / code / strikethrough like kramdown', () => {
    const md = 'Start **bold** and *em* and __strong__ and _emph_ and `code` and ~~del~~<br>';
    const ast = parse_inline_markdown(md);

    // first node is the formatted text
    expect(ast[0].type).toBe('text');
    expect(ast[0].content).toBe(
      'Start <strong>bold</strong> and <em>em</em> and <strong>strong</strong> and <em>emph</em> and <code>code</code> and <del>del</del><br>',
    );
  });

  it('parses links that contain formatting & HTML in their label', () => {
    const md = '[**bold** <span>html</span> text](https://example.com/_notHere_/**notHere**)<br>';
    const ast = parse_inline_markdown(md);

    const link = ast[0];
    expect(link.type).toBe('link');

    expect(link.href).toBe('https://example.com/_notHere_/**notHere**');
    const inner = link.content as InlineMarkdownElement[];
    const htmlInside = toHtml(inner);
    expect(htmlInside).toBe('<strong>bold</strong> <span>html</span> text');
  });

  it('parses kramdown for links', () => {
    const md = '[text bold](https://example.com){: .class1}<br>';
    const ast = parse_inline_markdown(md);
  ...
  it('parses images with kramdown', () => {
    const md = '![text](image.png){: .class1}';
    const ast = parse_inline_markdown(md);
  ...
  it('supports an image nested inside a link', () => {
    const md = '[![Alt Text](image.png)](https://example.com)<br>';
    const ast = parse_inline_markdown(md);
