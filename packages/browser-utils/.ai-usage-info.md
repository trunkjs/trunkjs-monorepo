...
### Startup-loader-aware Lit elements

```ts
import { LitElement } from 'lit';
import { StartupLoaderMixin, waitForReady, waitForVisual } from '@trunkjs/browser-utils';

class MyEl extends StartupLoaderMixin(LitElement) {
  runLevel = 'content';
...
await waitForReady({ target: element, dependsOn: ['configuration'] });
await waitForVisual({ target: element });
```

`runLevel` and `dependsOn` are programmatic component properties. The default runlevel is the element's tag name, so all instances of one component type form one startup barrier. Do not configure startup dependencies through HTML attributes.
...
- `waitForReady` / `waitForPreVisual` / `waitForVisual` integrate with the nearest local or global startup loader and fall back to `window.load` when none is active.
- `StartupLoaderMixin` snapshots its programmatic `runLevel` and `dependsOn` properties during connection. Await `super.connectedCallback()` when overriding it.
