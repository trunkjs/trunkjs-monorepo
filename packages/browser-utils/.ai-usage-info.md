...
await waitForReady({ target: element, dependsOn: ['config'] });
...
- `StartupLoaderMixin` reads `startup-id` and `depends-on` from the host element. Await `super.connectedCallback()` when overriding it.
